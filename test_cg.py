...
"""
Taichi Sparse Matrix CG
"""


def CG_taichi(A: ti.linalg.SparseMatrix, b, x0):
    m, n = A.num_rows(), A.num_cols()
...
def computeSv_taichi(complianceMatrix, G, A, v):
    Gv = G @ v
    x0 = np.zeros(2 * (N - 1), dtype=np.float64)
    AinvGV = CG_taichi(A, Gv, x0)
...
def CGwithCG_taichi(complianceMatrix, G, A, b, x0):
    m, n = A.num_rows(), A.num_cols()
    assert (m == n and b.shape == x0.shape)
    residual = 1.0e-6
    Sx0 = computeSv_taichi(complianceMatrix, G, A, x0)
    r0 = b - Sx0
    if np.linalg.norm(r0) < residual:
        return x0
    x = x0
    ...
    count = 0
    while count < MaxCGIte:
        Sp = computeSv_taichi(complianceMatrix, G, A, p)
        rr = sum(r * r)
        alpha = rr / sum(p * Sp)
        x = x + alpha * p
        r_next = r - alpha * Sp
...
@ti.kernel
def buildA(Abuilder: ti.types.sparse_matrix_builder(),
           mass: ti.types.ndarray(), KK: ti.types.ndarray()):
    # uppper left: mass matrix
    for i in range(N):
        Abuilder[2 * i, 2 * i] += mass[i]
        Abuilder[2 * i + 1, 2 * i + 1] += mass[i]
    # uppper left: geometric stiffness
    for i in range(N):
        for j in range(N):
            Abuilder[2 * i + 0, 2 * j + 0] += KK[i + 0, j + 0]
            Abuilder[2 * i + 1, 2 * j + 1] += KK[i + 1, j + 1]
            Abuilder[2 * i + 2, 2 * j + 2] += KK[i + 2, j + 2]
...
def solveWithSchurComplement(mass, p, prep, g, KK, l, c, cidx, iteration):
    ...
    # create left upper matrix
    Abuilder = ti.linalg.SparseMatrixBuilder(dim, dim, max_num_triplets=10000)
    buildA(Abuilder, mass, KK)
    A = Abuilder.build()
    ...
    dl = CGwithCG_taichi(complianceMatrix, G, A, b, x0)
